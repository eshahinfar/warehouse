...
-- Mirrors the current SQLite schema in src/db.js.
...
CREATE TABLE products (
  ...
  type VARCHAR(64) NOT NULL DEFAULT 'مصرفی',
...
CREATE TABLE transactions (
  ...
  source_doc_number VARCHAR(191) NULL,
  description TEXT NULL,
  created_by_user_id BIGINT UNSIGNED NULL,
  created_at VARCHAR(64) NOT NULL,
  status VARCHAR(64) NOT NULL DEFAULT 'معتبر',
  voided_at VARCHAR(64) NULL,
  void_reason TEXT NULL,
  voided_by_user_id BIGINT UNSIGNED NULL,
  PRIMARY KEY (id),
  UNIQUE KEY uq_transactions_doc_number (doc_number),
  KEY idx_transactions_product (product_id),
  KEY idx_transactions_date (date),
  KEY idx_transactions_status (status),
  KEY idx_transactions_created_by (created_by_user_id),
  KEY idx_transactions_source_doc_id (source_doc_id),
...
CREATE TABLE requests (
  ...
  quantity INT NOT NULL,
  date VARCHAR(64) NOT NULL,
  priority VARCHAR(64) NOT NULL DEFAULT 'عادی',
  status VARCHAR(64) NOT NULL DEFAULT 'در انتظار',
  notes TEXT NULL,
  auto TINYINT(1) NOT NULL DEFAULT 0,
  created_by_user_id BIGINT UNSIGNED NULL,
  created_at VARCHAR(64) NOT NULL,
  PRIMARY KEY (id),
  UNIQUE KEY uq_requests_doc_number (doc_number),
  KEY idx_requests_product (product_id),
  KEY idx_requests_status (status),
  KEY idx_requests_created_by (created_by_user_id),
...
CREATE TABLE custody_records (
  ...
  expected_return VARCHAR(64) NULL,
  condition_out TEXT NULL,
  notes TEXT NULL,
  status VARCHAR(64) NOT NULL DEFAULT 'باز',
  return_date VARCHAR(64) NULL,
  condition_in TEXT NULL,
  return_notes TEXT NULL,
  return_doc_number VARCHAR(191) NULL,
  created_by_user_id BIGINT UNSIGNED NULL,
  created_at VARCHAR(64) NOT NULL,
  void_reason TEXT NULL,
  PRIMARY KEY (id),
  UNIQUE KEY uq_custody_doc_number (doc_number),
  KEY idx_custody_product (product_id),
  KEY idx_custody_status (status),
  KEY idx_custody_created_by (created_by_user_id),
...
CREATE TABLE repair_records (
  ...
  product_id BIGINT UNSIGNED NOT NULL,
  quantity INT NOT NULL,
  send_date VARCHAR(64) NOT NULL,
  submitted_by VARCHAR(255) NOT NULL,
  repair_shop VARCHAR(255) NULL,
  issue_description TEXT NOT NULL,
  status VARCHAR(64) NOT NULL DEFAULT 'در حال تعمیر',
  result_date VARCHAR(64) NULL,
  result TEXT NULL,
  technician VARCHAR(255) NULL,
  approver VARCHAR(255) NULL,
  result_notes TEXT NULL,
  result_doc_number VARCHAR(191) NULL,
  source_custody_id BIGINT UNSIGNED NULL,
  created_by_user_id BIGINT UNSIGNED NULL,
  created_at VARCHAR(64) NOT NULL,
  void_reason TEXT NULL,
  PRIMARY KEY (id),
  UNIQUE KEY uq_repair_doc_number (doc_number),
  KEY idx_repair_product (product_id),
  KEY idx_repair_status (status),
  KEY idx_repair_created_by (created_by_user_id),
...
CREATE TABLE audit_log (
  ...
  KEY idx_audit_doc_number (doc_number),
...
('in', 0), ('out', 0), ('ret', 0), ('req', 0),
('custIssue', 0), ('custReturn', 0), ('repIn', 0), ('repOut', 0)
